// d3des.c : Data Encryption Standard
// 2013.5.24 v1.0
...
//3DES解密文件
int D3DES_Decrypt_Str(char *cipherFile, char *keyStr, ElemType **plainBuffer){   
    FILE *cipher,*key;
    ElemType *plainBlock,*cipherBlock,keyBlock[24];    
    int count;
    long fileLen_in;
    ...
    count = D3DES_Decrypt(cipherBlock,keyBlock,plainBlock,fileLen_in);
    // memcpy(plainBuffer,plainBlock,count-1);
    // memcpy(plainBuffer+count,"\0",1); 
    *plainBuffer = plainBlock;
